...
function ProfileTab() {
  ...
  const saveProfile = trpc.auth.saveProfile.useMutation({
    onSuccess: () => { toast.success("Profile saved!"); refetch(); },
    onError: (e) => toast.error(e.message || "Failed to save profile"),
  });

  const uploadPic = trpc.auth.uploadProfilePicture.useMutation({
...
function SubscriptionTab() {
  ...
  const subMutation = trpc.clientJobs.createCheckout?.useMutation?.() ?? null;
