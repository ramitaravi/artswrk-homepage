...
export default function ProJobDetail() {
  ...
  const unlockPro = () => startUpgrade({ audience: "artist", returnPath: jobUrl });
      ...
      <div className={`fixed bottom-0 left-0 right-0 bg-white border-t border-gray-100 px-5 py-4 flex items-center justify-between gap-4 z-40 shadow-[0_-4px_24px_rgba(0,0,0,0.07)] ${isAuthenticated ? "lg:hidden" : ""}`}>
        ...
        {canExternalApply ? (
          ...
          <button
            onClick={unlockPro}
            disabled={upgradePending}
            className="artist-grad-bg flex-shrink-0 px-6 py-3 rounded-2xl text-sm font-bold text-white hover:opacity-90 transition-opacity disabled:opacity-60 flex items-center gap-2"
          >
            {upgradePending ? <Loader2 size={14} className="animate-spin" /> : "🔒"}
            Unlock PRO
          </button>
        ) : (
          <button
