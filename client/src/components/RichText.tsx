...
export default function RichText({ html, className = "" }: RichTextProps) {
  // Emphasis pasted in as inline styles becomes real tags first — `style` is
  // stripped below, which would otherwise take the bold with it.
  const clean = DOMPurify.sanitize(toRenderableHtml(html), {
    ALLOWED_TAGS: [
