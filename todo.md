...
- [x] Build ApplyGateModal component: blurred job teaser (title, location, date, blurred description/budget), email input, submit arrow
- [x] tRPC endpoint: checkEmailExists — returns { exists: boolean } for a given email
- [x] If email exists → redirect to /login?next=/jobs/:locationSlug/:jobSlug/apply
