/**
 * update-ramita-profile.mjs
 * Updates Ramita's profile in the DB with data from Bubble artist record
 */
...
async function main() {
  ...
  // Data from Bubble artist record (1653048240176x373600625789868540)
  const artswrkPro = 1;
  ...
  const location = "Santa Clarita, CA, USA";
  const instagram = "https://www.instagram.com/ramita.ravi/";
  const tiktok = "https://www.tiktok.com/@ramitaravi?lang=en";
  const youtube = "https://www.youtube.com/channel/UCB60wNev05ZjiZZNix1iXTQ";
  const website = "https://www.ramitaravi.com";
  
  // Profile picture (new one from Bubble)
  ...
  // Portfolio photos (these are the "Media" photos shown in the screenshot)
  ...
  // Master artist types (Bubble IDs → we'll use display names)
  // From the screenshot: "Dance Adjudicator", "Dance Educator"
  const masterArtistTypes = JSON.stringify(["Dance Adjudicator", "Dance Educator"]);
  
  // Media photos
  const mediaPhotos = JSON.stringify(portfolioPhotos);
  
  console.log("Updating Ramita's profile...");
  console.log("Profile picture:", profilePicture);
  console.log("Media photos:", mediaPhotos);
  ...
  const [rows] = await conn.execute(
    "SELECT id, firstName, artswrkPro, pronouns, masterArtistTypes, profilePicture, mediaPhotos, resumeFiles FROM users WHERE id = 30001"
  );
  const r = rows[0];
  console.log("\nVerification:");
  ...
  console.log("  mediaPhotos:", r.mediaPhotos);
